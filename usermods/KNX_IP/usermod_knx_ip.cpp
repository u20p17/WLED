...
static LedProfile detectLedProfileFromSegments()
{
  ...
  if (lc == 0 && n > 0)
    lc = strip.getMainSegment().getLightCapabilities(); // fallback
  ...
  if (hasRGB && hasCCT)
    return LedProfile::RGBCCT;
  if (hasRGB && hasW)
    return LedProfile::RGBW;
  if (hasRGB)
    return LedProfile::RGB;
  if (hasCCT)
    return LedProfile::CCT;
  if (hasW)
...
static String g_gaConflictDetails;
static constexpr byte ERR_KNX_GA_CONFLICT = 33;
...
static void setPrimaryColorForSelectedSegments(uint8_t r, uint8_t g, uint8_t b, uint8_t w)
{
  const uint32_t color = RGBW32(r, g, b, w);
  const uint16_t segmentCount = strip.getSegmentsNum();
...
static uint16_t parseGA(const char *s)
{
  ...
  auto parseUInt = [&](uint32_t &out) -> bool
  {
    ...
    while (*p >= '0' && *p <= '9')
    {
      ...
      if (v > 1000u)
        return false; // simple overflow guard
      p++;
    }
    out = v;
    return true;
  ...
  if (!parseUInt(c) || *p != '\0')
    return 0;
  if (a > 31u || b > 7u || c > 255u)
    return 0; // KNX 3-level GA limits
...
static uint16_t parsePA(const char *s)
{
  ...
  if (!parseUInt(line) || *p != '.')
    return 0;
  ++p;
  if (!parseUInt(dev) || *p != '\0')
    return 0;
  if (area > 15u || line > 15u || dev > 255u)
    return 0; // strict
...
std::vector<uint16_t> KnxIpUsermod::getAllUsedGAs(bool includeRegisteredSegmentGAs) const
{
  ...
  if (includeRegisteredSegmentGAs)
  {
    // Include already registered segment GAs only for callers that need a full usage view.
    if (GA_SEG_IN_PWR)
    {
      for (uint8_t i = 0; i < numSegments; i++)
      {
        if (GA_SEG_IN_PWR[i] > 0)
          usedGAs.push_back(GA_SEG_IN_PWR[i]);
      }
    }
    if (GA_SEG_IN_BRI)
    {
      for (uint8_t i = 0; i < numSegments; i++)
      {
        if (GA_SEG_IN_BRI[i] > 0)
          usedGAs.push_back(GA_SEG_IN_BRI[i]);
      }
    }
    if (GA_SEG_IN_FX)
    {
      ...
      {
        if (GA_SEG_IN_FX[i] > 0)
          usedGAs.push_back(GA_SEG_IN_FX[i]);
      }
    }
    if (GA_SEG_OUT_PWR)
    {
      for (uint8_t i = 0; i < numSegments; i++)
      {
        if (GA_SEG_OUT_PWR[i] > 0)
          usedGAs.push_back(GA_SEG_OUT_PWR[i]);
      }
    }
    if (GA_SEG_OUT_BRI)
    {
      for (uint8_t i = 0; i < numSegments; i++)
      {
        if (GA_SEG_OUT_BRI[i] > 0)
          usedGAs.push_back(GA_SEG_OUT_BRI[i]);
      }
    }
    if (GA_SEG_OUT_FX)
    {
      for (uint8_t i = 0; i < numSegments; i++)
      {
        if (GA_SEG_OUT_FX[i] > 0)
          usedGAs.push_back(GA_SEG_OUT_FX[i]);
      }
    }
  }
...
bool KnxIpUsermod::hasGAConflicts(uint8_t maxSegments) const
{
  ...
  // Compare candidate segment GAs against central/static GAs, not against the
  // currently registered segment arrays, otherwise valid segment addresses can
  // be reported as conflicts with themselves after offsets change.
  std::vector<uint16_t> allGAs = getAllUsedGAs(false);
  ...
  if (hasConflicts)
  {
    g_gaConflictDetails = F("KNX GA conflicts: ");
    for (size_t i = 0; i < conflictList.size(); i++)
    {
      if (i > 0)
        g_gaConflictDetails += F(", ");
      g_gaConflictDetails += conflictList[i].c_str();
      if (g_gaConflictDetails.length() > 220)
        break;
    }
    if (!g_gaConflictDetails.endsWith("."))
      g_gaConflictDetails += F(". Check segment offsets.");

    // Debug: show what we put in the error details
    KNX_UM_DEBUGF("[KNX-UM] Conflict details set: '%s'\n", g_gaConflictDetails.c_str());
  }
...
void KnxIpUsermod::analyzeGAConflicts() const
{
  ...
  std::vector<uint16_t> allUsed = getAllUsedGAs(false);
  std::vector<uint16_t> centralGAs_parsed;
  ...
  for (uint8_t seg = 0; seg < std::min((uint8_t)segmentCount, (uint8_t)5); seg++)
...
// Log detailed conflict information for debugging
  ...
  else
  {
    ...
    extern byte errorFlag;
    if (errorFlag == ERR_KNX_GA_CONFLICT)
    {
...
void KnxIpUsermod::clearSegmentKOs()
{
  ...
  delete[] GA_SEG_IN_PWR;
  GA_SEG_IN_PWR = nullptr;
  delete[] GA_SEG_IN_BRI;
  GA_SEG_IN_BRI = nullptr;
  delete[] GA_SEG_IN_FX;
  GA_SEG_IN_FX = nullptr;

  delete[] GA_SEG_OUT_PWR;
  GA_SEG_OUT_PWR = nullptr;
  delete[] GA_SEG_OUT_BRI;
  GA_SEG_OUT_BRI = nullptr;
  delete[] GA_SEG_OUT_FX;
  GA_SEG_OUT_FX = nullptr;
  ...
  for (uint8_t seg = 0; seg < numSegments; seg++)
  {
        ...
        !(seg == 0 && GA_SEG_IN_BRI[seg] == centralInBri))
    {
      ...
      KNX.onGroup(GA_SEG_IN_BRI[seg],
                  ...
                  {
                    if (svc == KnxService::GroupValue_Write && p && len >= 1)
                      this->onKnxSegmentBrightness(seg, KnxIpCore::unpackScaling(p, len));
                  });
    }
...
{
  if (strip.getSegmentsNum() > 0)
    strip.getSegment(0).setMode(fxIndex);
  stateUpdated(CALL_MODE_DIRECT_CHANGE);
  scheduleStatePublish();
}
...
void KnxIpUsermod::adjustWhiteSplitRel(int16_t delta, bool adjustWarm)
{
  ...
  uint16_t sum = warm + cold;
  if (sum > 255)
    sum = 255; // clip combined
  uint8_t newW = (uint8_t)sum;
  uint8_t newCct;
  if (sum == 0)
    newCct = cct; // keep previous ratio if all off
  else
  {
    // cold fraction scaled to 0..255
    newCct = (uint8_t)((cold * 255u + sum / 2) / sum);
  }
  setPrimaryColorForSelectedSegments(r, g, b, newW);
  seg.cct = newCct;
...
void KnxIpUsermod::onKnxColorRel(uint8_t channel, uint8_t dpt3)
{
  ...
  *tgt = addClamp255(*tgt, d);
  setPrimaryColorForSelectedSegments(r, g, b, w);
  colorUpdated(CALL_MODE_DIRECT_CHANGE);
  scheduleStatePublish();
}
...
{
  int16_t d = knx_step_delta(dpt3 & 0x0F, 10);
  if (!d)
    return;
  int count = strip.getModeCount();
  int cur = (int)effectCurrent + d;
  if (cur < 0)
    cur = 0;
  else if (cur >= count)
    cur = count - 1;
  onKnxEffect((uint8_t)cur);
}
...
void KnxIpUsermod::onKnxWhite(uint8_t v)
{ // 0..255 (DPT 5.010)
  uint8_t r, g, b, w;
  getCurrentRGBW(r, g, b, w);
  setPrimaryColorForSelectedSegments(r, g, b, v);
  colorUpdated(CALL_MODE_DIRECT_CHANGE);
  scheduleStatePublish();
}

void KnxIpUsermod::onKnxCct(uint16_t kelvin)
{ // Kelvin (DPT 7.600)
  Segment &seg = strip.getSegment(0);
...
void KnxIpUsermod::applyWhiteAndCct()
{
  uint8_t r, g, b, w;
  getCurrentRGBW(r, g, b, w);
  setPrimaryColorForSelectedSegments(r, g, b, w); // W is already part of color; CCT is separate
  // CCT is stored on the segment
  colorUpdated(CALL_MODE_DIRECT_CHANGE);
}

void KnxIpUsermod::onKnxWW(uint8_t v)
{ // 0..255 (DPT 5.010)
...
void KnxIpUsermod::onKnxCW(uint8_t v)
{ // 0..255 (DPT 5.010)
  const Segment &seg = strip.getSegment(0);
  const uint8_t wLive = W(seg.colors[0]);
  const uint8_t cctLive = seg.cct;
...
void KnxIpUsermod::hsvToRgb(float h, float s, float v, uint8_t &r, uint8_t &g, uint8_t &b)
{
  if (s <= 0.f)
  {
    r = g = b = clamp8i((int)roundf(v * 255.f));
    return;
  }
  while (h < 0.f)
    h += 360.f;
...
{
  uint8_t r, g, b;
  hsvToRgb(hDeg, s01, v01, r, g, b);
  uint8_t cr, cg, cb, cw;
  getCurrentRGBW(cr, cg, cb, cw);
  if (!preserveWhite)
    cw = 0; // optional future use; currently always true
  setPrimaryColorForSelectedSegments(r, g, b, cw);
  colorUpdated(CALL_MODE_DIRECT_CHANGE);
  scheduleStatePublish();
}
...
{
  if (segmentIndex >= strip.getSegmentsNum())
    return;
...
void KnxIpUsermod::evalAndPublishTempAlarm(uint16_t ga, float tempC, float maxC, bool &lastState, const char *tag)
{
  ...
  if (!lastState && trip)
    newState = true; // 0 -> 1 (trip)
  else if (lastState && clear)
    newState = false; // 1 -> 0 (clear)
...
#if defined(ESP32)
  if (stopSntp)
    sntp_stop(); // optional: keep KNX authoritative
#endif
...
void KnxIpUsermod::setup()
{
  ...
  if (!GA_IN_PWR && *gaInPower)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in power '%s'\n", gaInPower);
  if (!GA_IN_BRI && *gaInBri)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in bri '%s'\n", gaInBri);
  if (!GA_IN_R && *gaInR)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in r '%s'\n", gaInR);
  if (!GA_IN_G && *gaInG)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in g '%s'\n", gaInG);
  if (!GA_IN_B && *gaInB)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in b '%s'\n", gaInB);
  if (!GA_IN_FX && *gaInFx)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in fx '%s'\n", gaInFx);
  if (!GA_IN_PRE && *gaInPreset)
    KNX_UM_WARNF("[KNX-UM][WARN] Invalid GA in preset '%s'\n", gaInPreset);
  ...
  Serial.printf("[KNX-UM] LED profile: %s (RGB=%d, W=%d, CCT=%d)\n",
                (g_ledProfile == LedProfile::MONO ? "MONO" : g_ledProfile == LedProfile::CCT ? "CCT"
                                                         : g_ledProfile == LedProfile::RGB   ? "RGB"
                                                         : g_ledProfile == LedProfile::RGBW  ? "RGBW"
                                                                                             : "RGBCCT"),
  ...
  if (!allowW)
  {
    GA_IN_W = GA_IN_W_REL = 0;
  }
  if (!allowCCT)
  {
    GA_IN_CCT = GA_IN_WW = GA_IN_CW = GA_IN_WW_REL = GA_IN_CW_REL = 0;
  }
  if (!(g_ledProfile == LedProfile::RGBW || g_ledProfile == LedProfile::RGBCCT))
  {
    GA_IN_RGBW = GA_IN_RGBW_REL = 0;
  }
  ...
  if (GA_IN_W)
  {
    register1ByteHandler(GA_IN_W, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxWhite(v); });
  }
  ...
  if (GA_IN_WW)
  {
    register1ByteHandler(GA_IN_WW, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxWW(v); });
  }
  ...
  if (GA_IN_CW)
  {
    register1ByteHandler(GA_IN_CW, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxCW(v); });
  }
  ...
  if (GA_IN_RGBW)
  {
    registerMultiHandler(GA_IN_RGBW, DptMain::DPT_251xx, 4, [this](const uint8_t *p)
                         { onKnxRGBW(p[0], p[1], p[2], p[3]); });
  }
  if (GA_IN_RGB_REL)
  {
    registerMultiHandler(GA_IN_RGB_REL, DptMain::DPT_232xx, 3, [this](const uint8_t *p)
                         { onKnxRGBRel(p[0], p[1], p[2]); });
  }
  if (GA_IN_HSV_REL)
  {
    registerMultiHandler(GA_IN_HSV_REL, DptMain::DPT_232xx, 3, [this](const uint8_t *p)
                         { onKnxHSVRel(p[0], p[1], p[2]); });
  }
  if (GA_IN_RGBW_REL)
  {
    registerMultiHandler(GA_IN_RGBW_REL, DptMain::DPT_251xx, 4, [this](const uint8_t *p)
                         { onKnxRGBWRel(p[0], p[1], p[2], p[3]); });
  }
  if (GA_IN_H)
  {
    register1ByteHandler(GA_IN_H, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxH(byteToHueDeg(v)); });
  }
  if (GA_IN_S)
  {
    register1ByteHandler(GA_IN_S, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxS(byteToPct01(v)); });
  }
  if (GA_IN_V)
  {
    register1ByteHandler(GA_IN_V, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxV(byteToPct01(v)); });
  }
  if (GA_IN_BRI_REL)
  {
  ...
  if (GA_IN_R_REL)
  {
    KNX.addGroupObject(GA_IN_R_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_R_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxColorRel(0,p[0]); });
  }
  if (GA_IN_G_REL)
  {
    KNX.addGroupObject(GA_IN_G_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_G_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxColorRel(1,p[0]); });
  }
  if (GA_IN_B_REL)
  {
    KNX.addGroupObject(GA_IN_B_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_B_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxColorRel(2,p[0]); });
  }
  if (GA_IN_W_REL)
  {
    KNX.addGroupObject(GA_IN_W_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_W_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxWhiteRel(p[0]); });
  }
  if (GA_IN_WW_REL)
  {
    KNX.addGroupObject(GA_IN_WW_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_WW_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxWWRel(p[0]); });
  }
  if (GA_IN_CW_REL)
  {
    KNX.addGroupObject(GA_IN_CW_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_CW_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxCWRel(p[0]); });
  }
  if (GA_IN_H_REL)
  {
    KNX.addGroupObject(GA_IN_H_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_H_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxHueRel(p[0]); });
  }
  if (GA_IN_S_REL)
  {
    KNX.addGroupObject(GA_IN_S_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_S_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxSatRel(p[0]); });
  }
  if (GA_IN_V_REL)
  {
    KNX.addGroupObject(GA_IN_V_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_V_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxValRel(p[0]); });
  }
  if (GA_IN_FX_REL)
  {
    KNX.addGroupObject(GA_IN_FX_REL, DptMain::DPT_3xx, false, true);
    KNX.onGroup(GA_IN_FX_REL, [this](uint16_t, DptMain, KnxService s, const uint8_t *p, uint8_t l)
                { if (s==KnxService::GroupValue_Write && l>=1) onKnxEffectRel(p[0]); });
  }
  ...
  if (GA_IN_BRI)
  {
    registerScalingHandler(GA_IN_BRI, [this](uint8_t pct)
                           { onKnxBrightness(pct); });
  }
  ...
  if (GA_IN_FX)
  {
    register1ByteHandler(GA_IN_FX, DptMain::DPT_5xx, [this](uint8_t v)
                         { onKnxEffect(v); });
  }
  ...
  if (GA_IN_PRE)
  {
    ...
    KNX.onGroup(GA_IN_PRE, [this](uint16_t, DptMain, KnxService svc, const uint8_t *p, uint8_t len)
                { if (svc==KnxService::GroupValue_Write && p && len>=1) onKnxPreset(p[0]); });
  }

  // Outbound/status objects
  if (GA_OUT_PWR)
    KNX.addGroupObject(GA_OUT_PWR, DptMain::DPT_1xx, /*tx=*/true, /*rx=*/false);
  if (GA_OUT_BRI)
    KNX.addGroupObject(GA_OUT_BRI, DptMain::DPT_5xx, true, false);
  if (GA_OUT_FX)
    KNX.addGroupObject(GA_OUT_FX, DptMain::DPT_5xx, true, false);
  if (GA_OUT_R)
    KNX.addGroupObject(GA_OUT_R, DptMain::DPT_5xx, true, false);
  if (GA_OUT_G)
    KNX.addGroupObject(GA_OUT_G, DptMain::DPT_5xx, true, false);
  if (GA_OUT_B)
    KNX.addGroupObject(GA_OUT_B, DptMain::DPT_5xx, true, false);
  if (GA_OUT_PRE)
    KNX.addGroupObject(GA_OUT_PRE, DptMain::DPT_5xx, true, false);
  if (GA_OUT_W)
    KNX.addGroupObject(GA_OUT_W, DptMain::DPT_5xx, true, false);
  if (GA_OUT_CCT)
    KNX.addGroupObject(GA_OUT_CCT, DptMain::DPT_7xx, true, false);
  if (GA_OUT_WW)
    KNX.addGroupObject(GA_OUT_WW, DptMain::DPT_5xx, true, false);
  if (GA_OUT_CW)
    KNX.addGroupObject(GA_OUT_CW, DptMain::DPT_5xx, true, false);
  if (GA_OUT_RGB)
    KNX.addGroupObject(GA_OUT_RGB, DptMain::DPT_232xx, true, false);
  if (GA_OUT_HSV)
    KNX.addGroupObject(GA_OUT_HSV, DptMain::DPT_232xx, true, false);
  if (GA_OUT_RGBW)
    KNX.addGroupObject(GA_OUT_RGBW, DptMain::DPT_251xx, true, false);
  if (GA_OUT_H)
    KNX.addGroupObject(GA_OUT_H, DptMain::DPT_5xx, true, false);
  if (GA_OUT_S)
    KNX.addGroupObject(GA_OUT_S, DptMain::DPT_5xx, true, false);
  if (GA_OUT_V)
    KNX.addGroupObject(GA_OUT_V, DptMain::DPT_5xx, true, false);
  if (GA_OUT_INT_TEMP)
    KNX.addGroupObject(GA_OUT_INT_TEMP, DptMain::DPT_14xx, true, false);
  if (GA_OUT_TEMP)
    KNX.addGroupObject(GA_OUT_TEMP, DptMain::DPT_14xx, true, false);
  if (GA_OUT_INT_TEMP_ALARM)
    KNX.addGroupObject(GA_OUT_INT_TEMP_ALARM, DptMain::DPT_1xx, /*tx=*/true, /*rx=*/false);
  if (GA_OUT_TEMP_ALARM)
    KNX.addGroupObject(GA_OUT_TEMP_ALARM, DptMain::DPT_1xx, /*tx=*/true, /*rx=*/false);
...
void KnxIpUsermod::publishState()
{
  ...
  if (_pendingTxPower && GA_OUT_PWR)
    KNX.write1Bit(GA_OUT_PWR, pwr); // DPT 1.001
  if (_pendingTxBri && GA_OUT_BRI)
    KNX.writeScaling(GA_OUT_BRI, pct); // DPT 5.001
  ...
  if (_pendingTxColor && anyColorOut && colorOutMode != 1)
  { // per-channel
    ...
    {
      uint8_t v = r;
      KNX.groupValueWrite(GA_OUT_R, &v, 1);
    }
    if (chG)
    {
      uint8_t v = g;
      KNX.groupValueWrite(GA_OUT_G, &v, 1);
    }
    if (chB)
    {
      uint8_t v = b;
      KNX.groupValueWrite(GA_OUT_B, &v, 1);
    }
    if (chW)
    {
      uint8_t v = w;
      KNX.groupValueWrite(GA_OUT_W, &v, 1);
    }
    if (chCct)
    {
    ...
    {
      uint8_t v = ww;
      KNX.groupValueWrite(GA_OUT_WW, &v, 1);
    }
    if (chCW)
    {
      uint8_t v = cw;
      KNX.groupValueWrite(GA_OUT_CW, &v, 1);
    }
  }

  if (_pendingTxColor && anyColorChanged && colorOutMode != 0)
  { // composites
    ...
    if (GA_OUT_H)
    {
      uint8_t hb = hueDegToByte(hDeg);
      KNX.groupValueWrite(GA_OUT_H, &hb, 1);
    }
    if (GA_OUT_S)
    {
      uint8_t sb = pct01ToByte(s01);
      KNX.groupValueWrite(GA_OUT_S, &sb, 1);
    }
    if (GA_OUT_V)
    {
      uint8_t vb = pct01ToByte(v01);
      KNX.groupValueWrite(GA_OUT_V, &vb, 1);
    }
  }
...
void KnxIpUsermod::loop()
{
  ...
  static IPAddress lastIpForKnx;
  static bool lastNetworkConnected = Network.isConnected();

  const bool networkConnected = Network.isConnected();
  if (lastNetworkConnected && !networkConnected)
  {
    KNX_UM_WARNLN("[KNX-UM][WARN] Network link lost. Closing KNX socket until connectivity returns.");
    if (KNX.running())
      KNX.end();
    lastIpForKnx = IPAddress();
    knxStartedLogged = false;
  }
  else if (!lastNetworkConnected && networkConnected)
  {
    KNX_UM_DEBUGLN("[KNX-UM] Network link restored. Reinitializing KNX transport.");
    lastIpForKnx = IPAddress(); // force a fresh multicast join even if DHCP reuses the same IP
    knxStartedLogged = false;
  }
    ...
    if (newProf != g_ledProfile)
    {
      KNX_UM_DEBUGF("[KNX-UM] LED profile changed %s -> %s. Re-registering KNX GAs now.\n",
                    (g_ledProfile == LedProfile::MONO ? "MONO" : g_ledProfile == LedProfile::CCT ? "CCT"
                                                             ...
                                                             : g_ledProfile == LedProfile::RGBW  ? "RGBW"
                                                                                                 : "RGBCCT"),
                    (newProf == LedProfile::MONO ? "MONO" : newProf == LedProfile::CCT ? "CCT"
                                                        : newProf == LedProfile::RGB   ? "RGB"
                                                        : newProf == LedProfile::RGBW  ? "RGBW"
                                                                                       : "RGBCCT"));
    ...
    if (cur && cur.toString() != String("0.0.0.0"))
    {
      if (lastIpForKnx != cur)
      {
        KNX_UM_DEBUGF("[KNX-UM] Network IP changed %s -> %s, refreshing KNX multicast membership...\n",
                      lastIpForKnx.toString().c_str(), cur.toString().c_str());
...
bool KnxIpUsermod::readFromConfig(JsonObject &root)
{
  ...
  if (autoEnableBrightness > 255)
    autoEnableBrightness = 255;
  ...
  if (segmentOffsetL > 31)
    segmentOffsetL = 31;
  if (segmentOffsetM > 7)
    segmentOffsetM = 7;
  if (segmentOffsetN > 255)
    segmentOffsetN = 255;

  // accept either "GA in"/"GA out" (what we save) or "in"/"out"
  JsonObject gIn = top["GA in"];
  if (gIn.isNull())
    gIn = top["in"];
  JsonObject gOut = top["GA out"];
  if (gOut.isNull())
    gOut = top["out"];
  ...
  GA_IN_PWR = NEW_IN_PWR;
  GA_IN_BRI = NEW_IN_BRI;
  GA_IN_R = NEW_IN_R;
  GA_IN_G = NEW_IN_G;
  GA_IN_B = NEW_IN_B;
  GA_IN_W = NEW_IN_W;
  GA_IN_CCT = NEW_IN_CCT;
  GA_IN_WW = NEW_IN_WW;
  GA_IN_CW = NEW_IN_CW;
  GA_IN_FX = NEW_IN_FX;
  GA_IN_PRE = NEW_IN_PRE;
  GA_IN_H = NEW_IN_H;
  GA_IN_S = NEW_IN_S;
  GA_IN_V = NEW_IN_V;
  GA_IN_RGB = NEW_IN_RGB;
  GA_IN_HSV = NEW_IN_HSV;
  GA_IN_RGBW = NEW_IN_RGBW;
  GA_IN_BRI_REL = NEW_IN_BRI_REL;
  GA_IN_R_REL = NEW_IN_R_REL;
  GA_IN_G_REL = NEW_IN_G_REL;
  GA_IN_B_REL = NEW_IN_B_REL;
  GA_IN_W_REL = NEW_IN_W_REL;
  GA_IN_WW_REL = NEW_IN_WW_REL;
  GA_IN_CW_REL = NEW_IN_CW_REL;
  GA_IN_H_REL = NEW_IN_H_REL;
  GA_IN_S_REL = NEW_IN_S_REL;
  GA_IN_V_REL = NEW_IN_V_REL;
  GA_IN_FX_REL = NEW_IN_FX_REL;
  GA_IN_RGB_REL = NEW_IN_RGB_REL;
  GA_IN_HSV_REL = NEW_IN_HSV_REL;
  GA_IN_RGBW_REL = NEW_IN_RGBW_REL;

  GA_OUT_PWR = NEW_OUT_PWR;
  GA_OUT_BRI = NEW_OUT_BRI;
  GA_OUT_R = NEW_OUT_R;
  GA_OUT_G = NEW_OUT_G;
  GA_OUT_B = NEW_OUT_B;
  GA_OUT_W = NEW_OUT_W;
  GA_OUT_CCT = NEW_OUT_CCT;
  GA_OUT_WW = NEW_OUT_WW;
  GA_OUT_CW = NEW_OUT_CW;
  GA_OUT_FX = NEW_OUT_FX;
  GA_OUT_PRE = NEW_OUT_PRE;
  GA_OUT_H = NEW_OUT_H;
  GA_OUT_S = NEW_OUT_S;
  GA_OUT_V = NEW_OUT_V;
  GA_OUT_RGB = NEW_OUT_RGB;
  GA_OUT_HSV = NEW_OUT_HSV;
  GA_OUT_RGBW = NEW_OUT_RGBW;
  ...
  else
  {
    ...
    {
      if (!KNX.rejoinMulticast())
      { // refresh IP_ADD_MEMBERSHIP + MULTICAST_IF :contentReference[oaicite:4]{index=4}
        KNX.end();
        KNX.begin();
      }
      // Optional primer
...
void KnxIpUsermod::addToJsonInfo(JsonObject &root)
{
  ...
  if (g_gaConflictDetails.length() > 0)
  {
    JsonArray gaConflicts = user.createNestedArray("KNX GA Conflicts");
    gaConflicts.add(g_gaConflictDetails);
    gaConflicts.add("");
  }

  // Try adding the GA table as HTML
  String gaTable = getGATableHTML();
  if (gaTable.length() > 0 && gaTable.length() < 25000)
  { // increased limit for comprehensive GA table with all types
...
static KnxIpUsermod knx_ip;
REGISTER_USERMOD(knx_ip);