...
public:
  ...
  std::vector<uint16_t> getAllUsedGAs(bool includeRegisteredSegmentGAs = true) const;
...
private:
  ...
  static inline uint8_t hueDegToByte(float hDeg)
  {
    while (hDeg < 0)
      hDeg += 360.f;
    while (hDeg >= 360)
      hDeg -= 360.f;
    return (uint8_t)roundf(hDeg * 255.f / 360.f);
  }
  static inline float byteToHueDeg(uint8_t hb) { return (hb * 360.f) / 255.f; }
  static inline uint8_t pct01ToByte(float p)
  {
    if (p < 0)
      p = 0;
    if (p > 1)
      p = 1;
    return (uint8_t)roundf(p * 255.f);
  }
